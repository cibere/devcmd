...
class EmbedPaginator(discord.ui.View):
    def __init__(
        self, *, user: Optional[discord.Member] = None, pages: list[discord.Embed]
    ) -> None:
        ...
        if self.user:
            if self.user.id != interaction.user.id:
                await interaction.response.send_message(
                    f"Your not {self.user.mention}... are you?", ephemeral=True
                )
                return False
            return True
        else:
    ...
    @discord.ui.button(
        ...
        disabled=True,
        custom_id="devcmd_text_paginator:first",
    )
    async def firstPageButton(
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.blurple,
        disabled=True,
        custom_id="devcmd_embed_paginator:left",
    )
    ...
    @discord.ui.button(
        ...
        disabled=True,
        custom_id="devcmd_embed_paginator:index",
    )
    async def indexButton(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        pass
    ...
    @discord.ui.button(
        emoji="➡️",
        style=discord.ButtonStyle.blurple,
        custom_id="devcmd_embed_paginator:right",
    )
    ...
    @discord.ui.button(
        emoji="⏭️",
        style=discord.ButtonStyle.blurple,
        custom_id="devcmd_text_paginator:last",
    )
    async def lastPageButton(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        self.rightButton.disabled = True
...
class TextPaginator(discord.ui.View):
    def __init__(
        self, *, user: Optional[discord.Member] = None, pages: list[str]
    ) -> None:
        super().__init__(timeout=None)
        self.user = user
        self.pages = pages
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.blurple,
        disabled=True,
        custom_id="devcmd_text_paginator:left",
    )
    async def leftButton(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.gray,
        disabled=True,
        custom_id="devcmd_text_paginator:index",
    )
    async def indexButton(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        emoji="➡️",
        style=discord.ButtonStyle.blurple,
        custom_id="devcmd_text_paginator:right",
    )
    async def rightButton(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
...
class ViewPaginator(discord.ui.View):
    def __init__(
        self, *, user: Optional[discord.Member] = None, pages: list[discord.ui.View]
    ) -> None:
    ...
    @classmethod
    async def generate(cls, user: discord.Member, pages: list[discord.ui.View]):
    ...
    async def clear_views(self) -> None:
        for item in self.children:
            if not item.custom_id.startswith("devcmd"):  # type: ignore
...
class ViewEmbedPaginator(discord.ui.View):
    def __init__(
        ...
        *,
        ...
        pages: list[list[discord.Embed, discord.ui.View]],  # type: ignore
    ) -> None:
    ...
    @classmethod
    async def generate(
        cls, user: discord.Member, pages: list[list[discord.Embed, discord.ui.View]]  # type: ignore
    ):
        paginator = ViewEmbedPaginator(user=user, pages=pages)
        await paginator.add_view(pages[0][1])  # type: ignore
        return paginator
    ...
    async def firstPageButton(
        ...
        self.indexButton.label = f"{self.current_page + 1}/{len(self.pages)}"
        await self.clear_views()
        await self.add_view(self.pages[self.current_page][1])  # type: ignore
        await interaction.response.edit_message(
            view=self, embed=self.pages[self.current_page][0]
        )
